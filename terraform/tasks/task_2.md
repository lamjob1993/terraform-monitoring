...
- Нужно написать Terraform IaC темплейт для раворачивания x14 пустых контейнеров Docker с ОС Debian.
  ...
  - **То есть с этого этапа держим мысленно в голове контейнеры за VM-ки** (мы это делаем для экономии ресурсов, потому что ждать на домашнем ноутбуке или ПК, пока Terraform задеплоит x10 VM, у которых по 5-10Gb SSD - это очень долго).
...
- Выключаем эту клон-виртуалку, оставляем в фоне и не трогаем до репозитория Ansible.
