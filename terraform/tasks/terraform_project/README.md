...
Код ниже готов к использованию. Если нужно адаптировать для виртуальных машин или добавить задания, напишите!
...
resource "docker_container" "dev_web" {
  ...
  ports {
    internal = 80
    external = var.environments["dev"].web_port
  }
  command = ["/bin/bash", "-c", "apt-get update && apt-get install -y nginx && nginx -g 'daemon off;'"]
}
resource "docker_container" "dev_db" {
