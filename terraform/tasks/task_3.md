...
### Создание инфраструктуры с Docker (такая инфра будет в рабочем облаке в банке)

**ВНИМАНИЕ!** Деплоем инфры занимается **Terraform**, а деплоем конфигов и запуском ПО занимается **Ansible**, но в данном задании **Terraform** выполняет роль и одного и второго инструмента. Это сделано для понимания более полного спектра инструментария **Terraform** (модули, циклы и т.д), так как **Ansible** мы пока еще не знаем.
