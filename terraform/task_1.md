...
#### 5. В файл `.terraformrc` вписываем следующий конфиг (долгий способ, но работает железобетонно, таким образом OpenTofu будет смотреть в скачанные вами провайдеры локально):
...
#### 6. Способ с альтернативными зеркалами. В файл `.terraformrc` вписываем следующий конфиг (для загрузки провайдеров [с вашего зеркала](https://terraform-registry-mirror.ru/) - способ быстрый и работает на территории РФ):
